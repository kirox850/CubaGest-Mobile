...
export default function PlanModal({ visible, onClose, user }) {
  ...
  const handleQvaPay = async (planKey) => {
    try {
      setLoading(true);
      ...
      Alert.alert("Error", "No se pudo conectar con QvaPay: " + (e.message || ""));
    } finally {
      setLoading(false);
  ...
  const handleWhatsApp = (planKey, priceUSD) => {
