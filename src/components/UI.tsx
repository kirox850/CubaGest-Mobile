...
// ─── SELECT NATIVO (equiv. <select> de la web: mismo marco que inp) ──────────
// En iOS abre el picker nativo del sistema, en Android el suyo — igual que los
// selects nativos de POS/Inventario/Dashboard en la web.
export const Sel = ({ items, value, onValueChange, style }: {
  ...
  return (
    <View style={[inp, { paddingHorizontal: 0, paddingVertical: 0, justifyContent: 'center', minWidth: 120 }, style]}>
      <Picker
        selectedValue={value}
        onValueChange={onValueChange}
        dropdownIconColor={colors.textMuted}
        itemTextStyle={{ color: colors.text }}
        style={{ color: colors.text, fontSize: 14, height: 38, backgroundColor: 'transparent' }}
      >
        {items.map(it => (
          <Picker.Item key={it.value} label={it.label} value={it.value} color={colors.text} />
        ))}
      </Picker>
    </View>
  );
